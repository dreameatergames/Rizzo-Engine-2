...
#include <setjmp.h>
	...
	#endif
	//* // mankrip - no 386 assembler, no MASM
	...
	#endif
	/*/// mankrip - no 386 assembler, no MASM
	#define id386	0 //*/// mankrip - no 386 assembler, no MASM
	#if id386
...
,	developer
	;
extern	qboolean
	host_initialized		// true if into command execution
	;
extern	int
	host_framecount	// incremented every frame, never reset
	;
extern	double
	host_frametime
,	realtime			// not bounded in any way, changed at start of every frame, never reset
	;
extern void
...
extern int
	minimum_memory
,	current_skill		// skill level for currently loaded level
						// (in case the user changes the cvar while the level is running, this reflects the level actually in use)
	;
//
// chase
//
...
,	Chase_Reset (void)
,	Chase_Update (void)
,	Chase_Contents (void) // mankrip - chasecam fix
	;
dfunction_t
	* ED_FindFunction (char *name)	// FrikaC - qcexec function
	;
