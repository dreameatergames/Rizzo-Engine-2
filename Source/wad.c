...
// 2001-09-12 Returning information about loaded file by Maddes  end
	...
	for (i=0, lump_p = wad_lumps ; i<wad_numlumps ; i++,lump_p++)
	{
		...
		// mankrip - begin
		{
			...
			SwapPic (pic);
			RemapBackwards_Pixels (pic->data, lump_p->size);
			continue;
		}
		if (!Q_strcmp (lump_p->name, "conchars"))
		{
			RemapBackwards_Pixels ((byte *)(wad_base + lump_p->filepos), lump_p->size);
			RemapColorKey_Pixels ((byte *)(wad_base + lump_p->filepos), lump_p->size);
			continue;
		}
		#if 0
		if (lump_p->type == TYP_QTEX)
		{
			Sys_Error ("Lump %s is QTEX\n", lump_p->name);
			continue;
		}
		if (lump_p->type == TYP_MIPTEX)
		{
			Sys_Error ("Lump %s is MIPTEX\n", lump_p->name);
			continue;
		}
		#endif
		// mankrip - end
