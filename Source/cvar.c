...
char *Cvar_CompleteVariable (char *partial)
{
	...
	if (len)
	{
		cvar_t		*cmd = cvar_vars;
		for ( ; cmd ; cmd=cmd->next)
		{
			char	*sub = cmd->name;
			int		i = Q_strlen (sub);
			for ( ; i; i--, sub++)
				if (!Q_strncmp (partial, sub, len))
					return cmd->name;
		}
	}
	#else
...
============
*/
extern char *MK_cleanftos (float f); // mankrip - reduced config file
void Cvar_SetValue (char *var_name, float value)
{
	char	val[32];

	sprintf (val, "%s", MK_cleanftos(value)); // mankrip - reduced config file
...
============
*/
qboolean HaveSemicolon (char *s); // mankrip - reduced config file
void Cvar_WriteVariables (FILE *f)
{
	...
	for (var = cvar_vars ; var ; var = var->next)
		if (var->archive)
		// mankrip - reduced config file - begin
		{
			Cmd_TokenizeString(var->string);
			if (Cmd_Argc() == 1 && !HaveSemicolon(var->string))
				fprintf (f, "%s %s\n", var->name, var->string);
			else
		// mankrip - reduced config file - end
			fprintf (f, "%s \"%s\"\n", var->name, var->string);
		} // mankrip - reduced config file
}
