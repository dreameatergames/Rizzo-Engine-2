...
//int             static_registered = 1;  // only for startup check, then set // mankrip - removing pop.lmp check - removed
...
/* // mankrip - removing pop.lmp check - removed - begin
...
*/ // mankrip - removing pop.lmp check - removed - end
...
void MSG_WriteAngle (sizebuf_t *sb, float f)
{
	...
	MSG_WriteByte (sb, (int)(f*256.0/360.0) & 255); // mankrip - improved 8-bit angles - edited
}
...
int COM_CheckParm (char *parm)
{
	...
	// mankrip - ugly hack to avoid com_argv being overwritten by memory leaks on the DC - begin
	#ifdef _arch_dreamcast
	static char	*safe_argv[MAX_NUM_ARGVS];
	static qboolean done = false;
	char * text = com_cmdline;
	...
	if (!done)
	{
		...
		for (i = 1 ; i < com_argc ; i++)
			safe_argv[i] = (byte *) malloc (sizeof (byte) * MAX_OSPATH);
	}
	for (i = 1 ; i < com_argc ; i++)
	{
		text = COM_Parse (text);
		Q_strcpy (safe_argv[i], com_token);
	}
	com_argv = safe_argv;
	#endif
	// mankrip - ugly hack to avoid com_argv being overwritten by memory leaks on the DC - end
...
void COM_CheckRegistered (void)
{
	...
	*/ // mankrip - removing pop.lmp check - removed - end
	// mankrip - removing pop.lmp check - begin
	...
	// mankrip - removing pop.lmp check - end
}
...
// reconstitute the command line for the cmdline externally visible cvar
	...
	com_argv = largv;
//	com_argv = argv; // mankrip - this kills the -safe switch
	...
	// is the element a pak file?
		...
		// look through all the pak file elements
			...
			for (i=0 ; i<pak->numfiles ; i++)
				if (!Q_strcasecmp (pak->files[i].name, filename)) // mankrip - fixed
				{       // found it!
...
//
	for (i=0 ; i<64 ; i++) // mankrip - edited
	{
		sprintf (pakfile, "%s/pak%i.pak", dir, i);
		pak = COM_LoadPackFile (pakfile);
		if (!pak)
			continue;//break; // mankrip - edited
		search = Hunk_Alloc (sizeof(searchpath_t));
