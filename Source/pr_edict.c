...
// mankrip - reduced savegames - begin
...
char *PR_UglyValueString (etype_t type, eval_t *val)
{
	...
	case ev_void:
		...
		break;
	case ev_float:
		sprintf (line, "%s", MK_cleanftos (val->_float)); // mankrip - reduced savegames
		break;
	case ev_vector:
		// mankrip - reduced savegames - begin
...
char *PR_GlobalString (int ofs)
{
	...
	if (!def)
		sprintf (line,"%i(??\?)", ofs); // compiler warning fix - edited
	else
	{
...
void ED_Write (FILE *f, edict_t *ed)
{
	...
	if (ed == EDICT_NUM (0)) // world entity
	{
		#ifndef _arch_dreamcast // this is not needed on the Dreamcast version, whose svs.serverflags are saved by Host_Savegame_f instead
		fprintf (f, "_serverflags %i\n", svs.serverflags);
		#endif
	}
	...
	// if the value is still all 0, skip the field
		...
		// mankrip - reduced savegames - begin
		{
			...
			if (d->type == ev_string && !strcmp (s, ""))
				continue;
			// these happens sometimes
			if (d->type == ev_float && !strcmp (s, "0"))
				continue;
			if (d->type == ev_vector && !strcmp (s, "0 0 0"))
				continue;
			...
			fprintf (f, "%s ", name);
			// don't write the \" characters around the value if not needed
			Cmd_TokenizeString (s);
			fprintf (f, (Cmd_Argc () != 1) ? "\"%s\"\n" : "%s\n", s);
		}
		// mankrip - reduced savegames - end
	}
...
void ED_WriteGlobals (FILE *f)
{
	...
	for (i=0 ; i<progs->numglobaldefs ; i++)
	{
		...
		// mankrip - reduced savegames - begin
		{
			eval_t * val = (eval_t *)&pr_globals[def->ofs];
			if (type == ev_string && !strcmp (pr_strings + val->string, ""))
				continue;
			if (type == ev_float && fabs (val->_float) < 0.000001)
				continue;
			if (type == ev_entity && val->edict == 0)
				continue;
			// write the name of the global
			fprintf (f,"%s ", name);
			// don't write the \" characters around the value if not needed
...
qboolean	ED_ParseEpair (void *base, ddef_t *key, char *s)
{
	...
	case ev_float:
		*(float *)d = Q_atof (s); // BlackAura (08-12-2002) - Changed atof() to Q_atof
		break;
	...
	case ev_vector:
		...
		for (i=0 ; i<3 ; i++)
		{
			while (*v && *v != ' ')
				v++;
			*v = 0;
			((float *)d)[i] = Q_atof (w); // BlackAura (08-12-2002) - Changed atof() to Q_atof
			w = v = v+1;
		}
		break;
...
====================
*/
char		prev_ed_name[256]; // mankrip
char		badfields[256*4]; // mankrip
char *ED_ParseEdict (char *data, edict_t *ent)
{
	...
	badfields[0] = 0; // mankrip - clear it
...
if (!strcmp(com_token, "angle"))
{
	...
	anglehack = false;
	// mankrip - rotating brushes - begin
	// We must not allow the world model to rotate, so here's a hack to prevent this
	if (ent->v.modelindex == 1) // world model
		if (!strcmp(com_token, "angles"))
			com_token[0] = '_'; // hack to skip the value
	// mankrip - rotating brushes - end
...
if (!strcmp(com_token, "light"))
	...
	// mankrip - serverflags bugfix - begin
	if (ent == EDICT_NUM (0)) // world entity
	{
		if (!strcmp (keyname, "_serverflags"))
...
// and are immediately discarded by quake
		...
		if (!key)
		{
			Q_strcat(badfields, va("'%s' is not a field\n", keyname)); // mankrip
		//	Con_Printf ("'%s' is not a field\n", keyname); // mankrip - removed
			continue;
		}
...
void ED_LoadFromFile (char *data)
{
	...
	prev_ed_name[0] = 0; // mankrip - clear it
// parse ents
	while (1)
	{
...
//
	...
	// look for the spawn function
		...
		{
			// mankrip - begin
			// don't print the same kind of entity over and over, it's annoying. (ex: waypoints)
			ddef_t	*d;
			int		*v;
			d = &pr_fielddefs[37]; // classname
			v = (int *)((char *)&ent->v + d->ofs*4);

			if (Q_strcmp(prev_ed_name, PR_ValueString(d->type, (eval_t *)v)))
			{
				Q_strcpy(prev_ed_name, PR_ValueString(d->type, (eval_t *)v));
			// mankrip - end
			Con_Printf ("No spawn function for:\n");
			ED_Print (ent);
		//		if (badfields[0]) Con_Printf(badfields); // mankrip
			} // mankrip
			ED_Free (ent);
			continue;
		}
...
===============
*/
void PR_LoadProgs (char *progsname) // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
{
	int		i;
...
// flush the non-C variable lookup cache
	...
	if (!progsname || !*progsname) // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
		progsname = "progs.dat"; // mankrip
	// 2001-09-12 Returning information about loaded file by Maddes  start
	fileinfo = COM_LoadHunkFile (progsname); // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
	// mankrip - begin
	if (!fileinfo)
	{
		Con_Printf ("PR_LoadProgs: couldn't load %s", progsname); // just warn the user
		fileinfo = COM_LoadHunkFile ("progs.dat"); // load the default one instead
	}
	// mankrip - end
	if (!fileinfo)
	// 2001-09-12 Returning information about loaded file by Maddes  end
		Sys_Error ("PR_LoadProgs: couldn't load %s", progsname); // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
	progs = (dprograms_t *)fileinfo->data;	// 2001-09-12 Returning information about loaded file by Maddes
	Con_DPrintf ("%s occupy %iK.\n", progsname, com_filesize/1024); // edited
...
// byte swap the header
	...
	if (progs->version != PROG_VERSION)
		Sys_Error ("%s has wrong version number (%i should be %i)", progsname, progs->version, PROG_VERSION); // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
	if (progs->crc != PROGHEADER_CRC)
		Sys_Error ("%s system vars have been modified, progdefs.h is out of date", progsname); // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
...
// 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes/Firestorm  end
	...
	// FrikaC - EndFrame function - begin
	EndFrame = 0;
