...
cvar_t sv_progs = {"sv_progs", "progs.dat"}; // Baker - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
cvar_t sv_makaqu = {"sv_makaqu", "0"}; // mankrip
cvar_t sv_compatibility = {"sv_compatibility", "0"}; // mankrip
cvar_t sv_qcexec = {"sv_qcexec","0", false, true}; // mankrip - qcexec
/*
===============
SV_Init
===============
...
void SV_Init (void)
{
	...
	extern	cvar_t	sv_aim_h; // mankrip - horizontal autoaim
	extern	cvar_t	sv_aim;
	extern	cvar_t	sv_enable_use_button; // mankrip - +USE fix
	...
	Cvar_RegisterVariable (&sv_aim_h); // mankrip - horizontal autoaim
	Cvar_RegisterVariable (&sv_aim);
	Cvar_RegisterVariable (&sv_nostep);
	Cvar_RegisterVariable (&sv_enable_use_button); // mankrip - +USE fix
	Cvar_RegisterVariable (&sv_makaqu); // mankrip
	Cvar_RegisterVariable (&sv_compatibility); // mankrip
	Cvar_RegisterVariable (&sv_qcexec); // mankrip - qcexec
	Cvar_RegisterVariable (&sv_progs); // Baker - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
...
void SV_SendServerinfo (client_t *client)
{
	...
	if (sv_compatibility.value) // mankrip
		MSG_WriteLong (&client->message, PROTOCOL_VERSION);
	else // mankrip
		MSG_WriteLong (&client->message, PROTOCOL_VERSION-1); // mankrip - 16-bit angles - edited
...
=============
*/
extern int current_protocol; // mankrip
...
//	float	glow_blue;
	...
	float	frame_interval=0.1f; // mankrip - QC frame_interval
...
// send an update
	...
	//
		...
		*/
		...
		{
			if (sv_makaqu.value)
				MSG_WriteByte (msg, (int)ent->v.effects >> 8);
			else
				MSG_WriteByte (msg, (int)(ent->v.effects - ((int)ent->v.effects & (EF_REFLECTIVE|EF_SHADOW|EF_CELSHADING))) >> 8);
		}
		// mankrip - end
	}
}
...
void SV_WriteClientdataToMessage (edict_t *ent, sizebuf_t *msg)
{
	...
	eval_t	*val;
#endif
	extern int current_protocol; // mankrip - high values in the status bar
...
// [always sent]	if (bits & SU_ITEMS)
	...
	} // mankrip - high values in the status bar
...
#endif
{
	edict_t		*ent;
	int			i;
	extern int current_protocol; // mankrip - 16-bit angles
...
// load progs to get entity field count
	PR_LoadProgs (sv_progs.string); // Baker - edited - http://forums.inside3d.com/viewtopic.php?f=12&t=1263
...
// leave slots at start for clients only
	...
	{
		sv.active = false;
		Host_Error/*Con_Printf*/ ("Couldn't spawn server %s\n", sv.modelname); // mankrip - edited
		return;
	}
	sv.models[1] = sv.worldmodel;

	// mankrip - 16-bit angles - begin
	if (sv_compatibility.value)
		current_protocol = PROTOCOL_VERSION;
	else
		current_protocol = PROTOCOL_VERSION-1;
	// mankrip - 16-bit angles - end
//
