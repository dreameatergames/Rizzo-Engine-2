...
static int		vid_stretched;//, windowed_mouse; // mankrip - removed
...
//cvar_t		_windowed_mouse = {"_windowed_mouse","0", true}; // mankrip - removed
...
void VID_InitMGLDIB (HINSTANCE hInstance)
{
    ...
    wc.lpszMenuName  = 0;
    wc.lpszClassName = "Makaqu"; // WinQuake // mankrip - edited
...
// for the rest of the session
	if (!vid_mode_set)
	{
		mainwindow = CreateWindowEx (
			 ExWindowStyle,
			 "Makaqu", // WinQuake // mankrip - edited
			 "Makaqu", // WinQuake // mankrip - edited
			 WindowStyle,
			 0, 0,
...
// empty while Quake starts up.
	...
	vid.maxwarpwidth = vid.width; // mankrip - hi-res waterwarp
	vid.maxwarpheight = vid.height; // mankrip - hi-res waterwarp
...
// so Con_Printfs don't mess us up by forcing vid and snd updates
	...
	{
		if (key_dest != key_menu && !cl.paused && !cls.demoplayback) // mankrip - edited
		{
...
//		Con_SafePrintf ("%s\n", VID_GetModeDescription (vid_modenum));
	...
	in_mode_set = false;
	vid.recalc_refdef = 1;
	SCR_Adjust(); // mankrip - screen positioning
...
//	if (r_dowarp) // mankrip - buffered video (bloody hack) - removed
		d_viewbuffer = r_warpbuffer;
//	else // mankrip - buffered video (bloody hack) - removed
//		d_viewbuffer = (void *)(byte *)vid.buffer; // mankrip - buffered video (bloody hack) - removed

//	if (r_dowarp) // mankrip - buffered video (bloody hack) - removed
		screenwidth = vid.width;//WARP_WIDTH; // mankrip - hi-res waterwarp - edited
//	else // mankrip - buffered video (bloody hack) - removed
//		screenwidth = vid.rowbytes; // mankrip - buffered video (bloody hack) - removed
...
//	Cvar_RegisterVariable (&_windowed_mouse); // mankrip - removed
	...
	if (!COM_CheckParm("-startwindowed")) // mankrip - hack to prevent crash when switching to fullscreen after starting up as windowed
	{ // mankrip - hack to prevent crash when switching to fullscreen after starting up as windowed
	if (!dibonly)
...
// fullscreen DIBs as well
	...
	// vid.width and vid.height hasn't been set yet
	vid.maxwarpwidth = WARP_WIDTH;
	vid.maxwarpheight = WARP_HEIGHT;
	vid.colormap = &colorshadingmap[WITH_BRIGHTS][256];//host_colormap; // mankrip - edited
...
// handle the mouse state when windowed if that's changed
	if (modestate == MS_WINDOWED)
	{
//		if (_windowed_mouse != windowed_mouse) // mankrip - edited
		{
			if (key_dest != key_menu && !cl.paused && !cls.demoplayback) // mankrip - edited
			{
				IN_ActivateMouse ();
...
//			windowed_mouse = _windowed_mouse; // mankrip - edited
		}
	}
}
...
// minimize/restore fulldib windows/mouse-capture normal windows on demand
	if (!in_mode_set)
	{
		if (ActiveApp)
		{
			...
			else if ((modestate == MS_WINDOWED) && key_dest != key_menu && !cl.paused && !cls.demoplayback) // mankrip - edited
			{
				IN_ActivateMouse ();
				IN_HideMouse ();
			}
...
void VID_HandlePause (qboolean pause)
{
	...
	if ((modestate == MS_WINDOWED)) // mankrip - edited
	if (key_dest != key_menu) // mankrip
	{
		if (pause)
		{
			IN_DeactivateMouse ();
...
extern void M_Video_f (void); // mankrip - edited
...
void VID_MenuDraw (void)
{
	...
	M_DrawTransPic ( (320-p->width)/2, 4, p); // mankrip - edited
