...
typedef struct joystick_s // mankrip
{
...
//    DWORD	dwControlMap[JOY_MAX_AXES];
    ...
    float       old_x, old_y, old_z, old_r, old_u, old_v; // mankrip

    JOYINFOEX	ji;
} joystick_t; // mankrip

joystick_t joystick[2]; // mankrip
...
void IN_MouseEvent (int mstate)
{
	...
	if (/*mouseactive && */!dinput) // mankrip - edited
	{
...
void IN_Accumulate (void)
{
	...
	if (mouseactive)
	{
		if (!dinput)
		{
			if (key_dest != key_menu) // mankrip - disable analog axis input while in the menu
			{ // mankrip - disable analog axis input while in the menu
			GetCursorPos (&current_pos);
...
//		mouse_oldbuttonstate = 0;
	...
	mouse_oldbuttonstate = 0; // mankrip
}
...
void IN_StartupJoystick_ (int j, int j1)
{
	...
	{
	    if (joystick[j].joy_id == j1) // mankrip
            continue; // mankrip
	...
	{
		// mankrip - begin
		if (j) // not the first joystick
		//	Con_Printf ("\nJoystick %i not found (%x)\n\n", j + 1, mmr);
			Con_Printf ("\nJoystick %i not found\n\n", j + 1);
		else
		// mankrip - end
	...
	if ((mmr = joyGetDevCaps (joystick[j].joy_id, &jc, sizeof(jc))) != JOYERR_NOERROR)
	{
		// mankrip - begin
		if (j) // not the first joystick
			Con_Printf ("\nJoystick %i not found -- invalid joystick capabilities (%x)\n\n", j + 1, mmr);
		else
		// mankrip - end
...
void IN_StartupJoystick (void)
{
    ...
    if (joystick[0].joy_avail == true) // only tries the second one if a first one has been found
...
void Joy_AdvancedUpdate (int j)
{
	...
	int	i;
	if (joystick[j].joy_avail == false) // mankrip
		return; // mankrip
/*	DWORD dwTemp;
...
//    Con_Printf ("\nJoystick \"%s\" %i configured\n\n", joy_name.string, j + 1); // mankrip - edited
	if (key_dest == key_console) // mankrip
		Con_Printf ("\nJoystick %i configured\n\n", j + 1); // mankrip - edited
}
void Joy_AdvancedUpdate_f (void)
{
	...
	if (joystick[j].joy_haspov)
	{
		...
		if(joystick[j].ji.dwPOV != JOY_POVCENTERED)
		{
			if (joystick[j].ji.dwPOV == JOY_POVFORWARD)
				...
				povstate |= 0x08;
            // mankrip - POV diagonals support - begin
            ...
            // mankrip - POV diagonals support - end
		}
...
void Joy_UpdateAxis (usercmd_t *cmd, int mode, float scale, float rawvalue, float threshold, float speed, float aspeed)
{
    ...
    if (joy_wwhack2.value != 0.0)
        if (mode == AXIS_TURN_L || mode == AXIS_TURN_R) // mankrip - edited
        {
...
/*
    ...
    case AxisTurn:
        if (fabs(fAxisValue) > joy_yawthreshold.value)
        {
            if(j->dwControlMap[i] == JOY_ABSOLUTE_AXIS)
                cl.viewangles[YAW] += (fAxisValue * joy_yawsensitivity.value/fovscale) * aspeed * cl_yawspeed.value; // mankrip - FOV-based scaling - edited
            else
                cl.viewangles[YAW] += (fAxisValue * joy_yawsensitivity.value/fovscale) * speed * 180.0; // mankrip - FOV-based scaling - edited
        }
        break;
    ...
    case AxisLook:
        if (fabs(fAxisValue) > joy_pitchthreshold.value)
        {
            ...
            if(j->dwControlMap[i] == JOY_ABSOLUTE_AXIS)
                cl.viewangles[PITCH] += (fAxisValue * joy_pitchsensitivity.value/fovscale) * aspeed * cl_pitchspeed.value; // mankrip - FOV-based scaling - edited
            else
                cl.viewangles[PITCH] += (fAxisValue * joy_pitchsensitivity.value/fovscale) * speed * 180.0; // mankrip - FOV-based scaling - edited
            V_StopPitchDrift();
        }
        else
            // no pitch movement
...
void IN_JoyMove_ (usercmd_t *cmd, int i)
{
	...
	{
		Joy_AdvancedUpdate (i); // mankrip - edited
	...
	if (IN_ReadJoystick_ (i) != true) // mankrip - edited
		return;
