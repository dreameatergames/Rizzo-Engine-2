...
void SV_AddGravity (edict_t *ent)
{
	...
	if (val && val->_float)
		...
		ent_gravity = 1.0;
#endif
	ent->v.velocity[2] -= ent_gravity * sv_gravity.value * host_frametime * 0.5f; // mankrip - ticrate-independent acceleration
}
...
// see if any solid entities are inside the final position
	...
	for (e=1 ; e<sv.num_edicts ; e++, check = NEXT_EDICT(check))
	{
		...
		if (check->v.movetype == MOVETYPE_PUSH
		|| check->v.movetype == MOVETYPE_NONE
	//	|| check->v.movetype == MOVETYPE_FOLLOW // mankrip - rotating brushes - removed
		|| check->v.movetype == MOVETYPE_NOCLIP)
			continue;
...
void SV_Physics_Pusher (edict_t *ent)
{
	...
	// mankrip - rotating brushes - edited - begin
	if (ent->v.avelocity[0] || ent->v.avelocity[1] || ent->v.avelocity[2])
		SV_PushRotate (ent, movetime);

	if (movetime)
		SV_PushMove (ent, movetime);	// advances ent->v.ltime if not blocked
	// mankrip - rotating brushes - edited - end
...
#endif
		// mankrip - ticrate-independent acceleration - begin
		if (!SV_CheckWater (ent) && ! ((int)ent->v.flags & FL_WATERJUMP) )
			SV_AddGravity (ent);
		// mankrip - ticrate-independent acceleration - end
		break;
...
// freefall if not onground
	if ( ! ((int)ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM) ) )
	{
		...
		SV_CheckVelocity (ent);
		SV_FlyMove (ent, host_frametime, NULL);
		if ( ! ((int)ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM) ) ) // mankrip - ticrate-independent acceleration
			SV_AddGravity (ent); // mankrip - ticrate-independent acceleration
		SV_LinkEdict (ent, true);
...
#if 1	// mankrip - removed - begin
		if (pr_global_struct->force_retouch)
		{
			SV_LinkEdict (ent, true);	// force retouch even for stationary
		}
#endif	// mankrip - removed - end
...
//		}
		SV_AddGravity (tent); // mankrip - ticrate-independent acceleration
