...
// build the sprite poster in worldspace
	// mankrip - QC Scale - edited - begin
	VectorScale (r_spritedesc.vright,	r_spritedesc.pspriteframe->right*currententity->scale2*currententity->scalev[0], right);
	VectorScale (r_spritedesc.vright,	r_spritedesc.pspriteframe->left	*currententity->scale2*currententity->scalev[0], left);

	VectorScale (r_spritedesc.vup,		r_spritedesc.pspriteframe->up	*currententity->scale2*currententity->scalev[1], up);
	VectorScale (r_spritedesc.vup,		r_spritedesc.pspriteframe->down	*currententity->scale2*currententity->scalev[1], down);
	// mankrip - QC Scale - edited - end
...
mspriteframe_t *R_GetSpriteframe (msprite_t *psprite)
{
	...
	else
	{
		...
		// when loading in Mod_LoadSpriteGroup, we guaranteed all interval values are positive,
		// so we don't have to worry about division by 0
