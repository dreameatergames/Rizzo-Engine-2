...
int			vstartscan;
int			veryclose; // mankrip

// FIXME: should go away
extern void			R_RotateBmodel (void);
extern void			R_TransformFrustum (void);
extern void D_DrawSpans_dither (espan_t *pspan, int dither); // mankrip
...
int D_MipLevelForScale (float scale)
{
	int		lmiplevel;
	// mankrip - begin
	float f = minmaxclamp (0.0f, d_dithertexture.value, 1.0f);
	veryclose = (d_dithertexture.value >= 2.0f); // if 2, all miplevels will be dithered
	if (scale * f >= d_scalemip[0] )
	{
		veryclose = true;
		lmiplevel = 0;
	}
...
// TODO: could preset a lot of this at mode set time
	if (r_drawflat.value)
	{
		for (s = &surfaces[1] ; s<surface_p ; s++)
		{
			...
			// mankrip - begin
			if (r_overdraw)
				if (s->flags & SURF_DRAWBACKGROUND) // shouldn't overdraw
					continue;
			// mankrip - end
	...
	else
	{
		...
		//		continue;
			...
			{
			//	if (!r_drawskybox) // mankrip - skyboxes
				D_DrawSkyScans (pspans); // mankrip
				D_ClearZSpans (pspans); // mankrip
			}
			...
			{
				extern byte	*r_skypixels[6]; // mankrip - hi-res skyboxes - edited
			...
			// FIXME: make this passed in to D_CacheSurface
				...
				{
					D_DrawSpans16_Blend (pspans, veryclose);
				}
				else
				{
					// mankrip - begin
					if (veryclose)
						D_DrawSpans_dither (pspans, veryclose);
					else
						D_DrawSpans16 (pspans);
					// mankrip - end
				}
