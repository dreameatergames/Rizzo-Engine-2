...
{
	/* // mankrip - "quit" always quits - removed - begin
	if (key_dest != key_console && cls.state != ca_dedicated)
	{
		M_Menu_Quit_f ();
		return;
	}
	*/ // mankrip - "quit" always quits - removed - end
...
==================
*/
extern cvar_t sv_qcexec; // mankrip
void Host_QC_Exec (void)
{
	...
	if (!(sv_qcexec.value || developer.value)) // mankrip
		return;
	if ((f = ED_FindFunction(Cmd_Argv(1))) != NULL)
	{
		...
		Con_Printf("qcexec: bad function \"%s\"\n", Cmd_Argv(1)); // mankrip - edited
...
void Host_Map_f (void)
{
	...
	// BlackAura (09/08/2004) - Turn off the music
...
void Host_Changelevel_f (void)
{
	...
	SV_SaveSpawnparms ();
	strcpy (level, Cmd_Argv(1));
	// mankrip - map transition lists - begin
	{
	...
	// mankrip - map transition lists - end
...
//
//======================================
char *MK_cleanftos (float f); // mankrip - reduced savegames
...
// mankrip - VMU saves - end
	...
	Con_Printf ("Saving game to %s...", name); // edited
	f = fopen (name, "wb"); // mankrip - reduced savegames - edited
	if (!f)
	{
		Con_Printf ("\nERROR: couldn't open.\n"); // edited
		return;
	...
	for (i=0 ; i<NUM_SPAWN_PARMS ; i++)
		fprintf (f, "%s\n", MK_cleanftos(svs.clients->spawn_parms[i])); // mankrip - reduced savegames
	fprintf (f, "%d\n", current_skill);
	fprintf (f, "%s\n", sv.name);
	fprintf (f, "%i\n", svs.serverflags);
...
// mankrip - VMU saves - begin
	...
	VMU_Save (name, COM_SkipPath(name), "Makaqu game     ", description);
	}
#endif
// mankrip - VMU saves - end
	Con_Printf (" done.\n"); // edited
...
// mankrip - VMU saves - end
	sprintf (name, "%s/%s", com_gamedir, Cmd_Argv(1));
#endif // mankrip - VMU saves
	COM_DefaultExtension (name, ".SAV"); // mankrip - upper-case savegame filenames - edited
	...
	fprintf (f, "%s\n", MK_cleanftos(sv.time)); // mankrip - reduced savegames
	// mankrip - serverflags bugfix - begin
	#ifdef _arch_dreamcast
	fprintf (f, "%i\n", svs.serverflags); // hack to make new saves compatible with old versions of Makaqu for the Dreamcast
	#endif
...
// mankrip - VMU saves - begin
#ifdef _arch_dreamcast
	fprintf (f,".\n"); // hack to ensure Host_Loadgame_f won't read beyond the file
#endif
// mankrip - VMU saves - end
	fclose (f);
// mankrip - VMU saves - begin
	...
	char description[33];
	sprintf(description, "%-32s", cl.levelname);
	VMU_Save (name, COM_SkipPath(name), "Makaqu state    ", description);
	}
#endif
// mankrip - VMU saves - end
	Con_Printf (" done.\n"); // edited
...
void Host_Loadgame_f (void)
{
	...
	int		i;//, r; // mankrip - removed
	char	r;		// mankrip - VMU saves
	edict_t	*ent;
	int		entnum;
	int		version;
...
#else // mankrip - begin
	tfloat = 0; // compiler warning fix
	current_skill = DC_ScanInt(f); // Replacing fscanf
	Cvar_SetValue ("skill", current_skill);
#endif // mankrip - end
...
//	fscanf (f, "%f\n",&time);
	...
	// mankrip - serverflags bugfix - begin
	// hack to load old Makaqu's bugfixed saves.
	#ifndef _arch_dreamcast // this hack is not needed on the Dreamcast version, so let's comment this check out to avoid unnecessary slowdown on the loading

	// load everything up to the line where the first opening brace should be.
	// if it's not there, and is on the following line instead, this is an old Makaqu bugfixed save.
	for (i = 0 ; i < MAX_LIGHTSTYLES + 1 ; i++) // skip the lightstyles and try to load the first opening brace
		DC_ScanString (f, str);
	if (str[0] != '{') // no brace here, so it's not a vanilla save version
	{
		DC_ScanString (f, str); // see if the first opening brace is on the next line
		if (str[0] == '{') // the brace is here,
			version = -1; // so it's an old Makaqu save version
	}
	// close and reopen the file, to reset the handle
	fclose (f);
	f = fopen (name, "r");
	// skip the version, the comment, the spawnparms, the skill, the mapname and the time
	for (i = 0 ; i < NUM_SPAWN_PARMS + 5 ; i++)
		DC_ScanString (f, str);

	if (version == -1) // if it's an old Makaqu save version,
	#endif
		svs.serverflags = DC_ScanInt (f); // this is where its svs.serverflags value is stored
	// mankrip - serverflags bugfix - end
	CL_Disconnect_f ();
...
// mankrip - VMU saves - begin
#ifdef _arch_dreamcast
			// it seems feof isn't working for Dreamcast's /ram/ files, so let's just stop here
			break;
#else
// mankrip - VMU saves - end
		{
			Host_Error ("First token isn't a brace"); // mankrip - edited
			return; // mankrip
		}
#endif // mankrip - VMU saves
...
DELETE FILE (savegame / savestate / demo / screenshot)

===============================================================================
*/

// mankrip - begin
void Host_Delete_f (void)
{
	char	name[MAX_QPATH];
	...
	sprintf (name, "%s", COM_SkipPath(Cmd_Argv(1)));
	if (VMU_Delete (name))
#else
	FILE	*f;
	int		F;
	...
	sprintf (name, "%s/%s", com_gamedir, COM_SkipPath(Cmd_Argv(1)));
	f = fopen (name, "wb");
	if (!f)
	{
...
void Host_Pause_f (void)
{
	...
	else
	{
		...
		if (svs.maxclients > 1 || key_dest != key_game) // mankrip
		{ // mankrip
		if (sv.paused)
		{
...
void Host_Spawn_f (void)
{
	...
	client_t	*client;
	edict_t	*ent;
	float *sendangle; // mh - savegame viewangles fix
...
// with a permanent head tilt
	...
	sendangle = sv.loadgame ? ent->v.v_angle : ent->v.angles; // mh - savegame viewangles fix
	for (i=0 ; i < 2 ; i++)
		MSG_WriteAngle (&host_client->message, sendangle[i]); // mh - savegame viewangles fix - edited
...
void Host_Startdemos_f (void)
{
	...
	#if 1
	// clear demo list
	memset (cls.demos, 0, MAX_DEMONAME * MAX_DEMOS);
	...
	for (i = 1 ; i < c + 1 ; i++)
	{
		...
		fclose (f);
		f = NULL;
		strncpy (cls.demos[i - i2 - 1], Cmd_Argv (i), MAX_DEMONAME - 1);
	}
	democount = c - i2;
	#else
	// mankrip - end
...
void Host_Demos_f (void)
{
	if (cls.state == ca_dedicated)
		return;
	if (cls.demonum == -1)
		cls.demonum = 0;//1; // mankrip - edited
	CL_Disconnect_f ();
	CL_NextDemo ();
}
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand ("loadsmall", Host_SmallLoadgame_f); // mankrip - small saves
	Cmd_AddCommand ("savesmall", Host_SmallSavegame_f); // mankrip - small saves
	Cmd_AddCommand ("del", Host_Delete_f); // mankrip
