...
//int main (); // mankrip
//extern unsigned char *hunk_base; // mankrip
void Sys_Quit (void)
{
	// BlackAura (08-12-2002) - Reboot to main system menu
	Host_Shutdown();

//	free(hunk_base); // mankrip
//	main (); // mankrip
...
char dc_basedir[16] = "/cd/quake"; // mankrip
...
extern void HertzMenu (void); // mankrip

int mem_size = 10*1024*1024; // mankrip
...
int main ()
{
	...
	vid_clear(0, 0, 0);
	parms.memsize = mem_size; // mankrip - edited
