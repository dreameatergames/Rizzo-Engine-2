...
int		modmenu_argc = 0;
char	*modmenu_argv[MAX_NUM_ARGVS]; // mankrip - edited
...
typedef struct modentry_s
{
	...
	char	game_args[256];
	char	game_text[1024];
	int		sound, video, heapsize; // mankrip
} modentry_t;
...
static void ModList_Clear()
{
	...
	modlist_c = 0;
	for (i=0; i<MAX_MODLIST; i++) // mankrip
		modlist[i].sound = 11025; // mankrip
}
...
static void ModList_LoadIniFile(int c)
{
	...
	FILE		*f;
	// Open .ini file
	if ( !(f = fopen (va("%s/makaqu.ini", modlist[c].fullpath), "r")))
		f = fopen (va("%s/nxmakaqu.ini", modlist[c].fullpath), "r"); // old name
	if (f)
	{
		while (!feof(f))
		{
...
static void ModList_ReScan(const char *base)
{
	...
	while((ent = fs_readdir(dir)) != NULL)
	{
		...
		ModList_LoadIniFile(modlist_c); // mankrip
		modlist_c++;
	}
	fs_close(dir);
	// mankrip - added /ID1 support - begin
id1:
...
mods:
	// mankrip - added /ID1 support - end
	dir = fs_open(base, O_RDONLY | O_DIR);
	if(dir == -1) // mankrip
		return; // mankrip

	// mankrip - added mission packs - begin
	...
	fs_close(dir);
	dir = fs_open(base, O_RDONLY | O_DIR);
	// mankrip - added mission packs - end
...
static void ModMenu_Select(int i)
{
	...
	if(modlist[i].game_args[0]) // mankrip - edited
		sprintf(commandLine, " %s", modlist[i].game_args); // mankrip - edited
	else
		commandLine[0] = 0; // mankrip
	...
	for(n = 0; n < slcl; n++)
	{
		if(commandLine[n] <= ' ') // mankrip - edited
		{
			// mankrip - begin
			do
				commandLine[n++] = 0;
...
static void ModMenu_Draw()
{
	...
	for(i = md_top; i < md_bottom; i++)
	{
		if(i < modlist_c)
		{
			...
			ModMenu_DrawText(3, (i - md_top) + 2, tstring); // mankrip - edited
		}
	}
	ModMenu_DrawText(1, (md_select - md_top) + 2, ">"); // mankrip - edited

	if(stricmp(modlist[md_select].game_name, modlist[md_select].game_text)) // mankrip - show description
		ModMenu_DrawText_Center(0, 14, modlist[md_select].game_text); // mankrip - show description
//	ModMenu_DrawText_Center(0, 15, modlist[md_select].game_args); // mankrip - debug
	...
	if((dev = maple_enum_type(0, MAPLE_FUNC_CONTROLLER)))
		if ((state = (cont_state_t *)maple_dev_status(dev)))
			if(state->buttons & CONT_X)
				button = CONT_X;
	if(modlist_c == 1 && !button) // mankrip - edited
//	if(modlist_c <= 1)
	{
		ModMenu_Select(0);
		ModList_Free();
	...
	// wait for the user to release the Start button, so the game doesn't start accidentally
	while (state->buttons & (CONT_A | CONT_START))
	{
		if ( (dev = maple_enum_type (0, MAPLE_FUNC_CONTROLLER)))
	...
	// Init KOS stuff
    vid_set_mode ( (dc_50hz ? DM_640x480_PAL_IL : DM_640x480), PM_RGB565); // mankrip
	...
	while(!finished)
	{
		...
		if(dev)
		{
			...
			if(state)
			{
				...
				else if(state->buttons & (CONT_A | CONT_START))
				{
...
// mankrip - begin
void HertzMenu_Display (void)
{
	// Clear the buffer
	MMD_CLS ();

    vid_set_mode ( (dc_50hz ? DM_640x480_PAL_IL : DM_640x480), PM_RGB565);

	ModMenu_DrawText_Center (0, 1, "Press Start to begin,");
	ModMenu_DrawText_Center (0, 3, "Press X + Start to open the game selection menu,");
	ModMenu_DrawText_Center (0, 5, "Or press any other button to switch frequency");
	ModMenu_DrawText_Center (0, 9, (dc_50hz ? "Current setting: 50Hz" : "Current setting: 60Hz"));
...
void HertzMenu (void)
{
	...
	// if the flashrom says the Dreamcast is European and a VGA Box is not hooked up, or if the Y button is held,
	if ((flashrom_get_region () == FLASHROM_REGION_EUROPE && vid_check_cable () != CT_VGA) || button & CONT_Y)
	{
		dc_50hz = (flashrom_get_region () == FLASHROM_REGION_EUROPE); // only defaults to 50 Hz on European DCs
		bbuffer = (unsigned short *)malloc (640 * 480 * 2); // 16-bit buffer
		// Prompt the user for whether to run in PAL50 or PAL60
		HertzMenu_Display ();
		while (true)
		{
			if ( (dev = maple_enum_type (0, MAPLE_FUNC_CONTROLLER)))
				if ( (state = (cont_state_t *)maple_dev_status (dev)))
				{
					...
					else if (button != state->buttons)
					{
						...
						if (button & (
							...
							|	CONT_C
							|	CONT_D
						//	|	CONT_X // ModMenu activator
						//	|	CONT_Y // HertzMenu activator
							|	CONT_Z
							|	CONT_DPAD_UP
							|	CONT_DPAD_DOWN
							|	CONT_DPAD_LEFT
