...
// 2001-11-15 DarkPlaces general builtin functions by LordHavoc  end
	...
	"DP_SND_DIRECTIONLESSATTNNONE",	// mankrip - sounds with no attenuation full volume on both sides?

	"DP_SV_SETCOLOR",	// 2001-11-15 DarkPlaces general builtin functions by LordHavoc
	"DP_SV_ROTATINGBMODEL",
	"FRIK_FILE",
	"MK_EF_SHADOW",		// mankrip
...
void PF_vectoyaw (void)
{
	...
	{
		yaw = /*(int)*/ (atan2(value1[1], value1[0]) * 180 / M_PI); // mankrip - 16-bit angles - edited
		if (yaw < 0)
			yaw += 360;
	}
...
void PF_vectoangles (void)
{
	...
	else
	{
		...
		pitch = /*(int)*/ (atan2(value1[2], forward) * 180 / M_PI); // mankrip - 16-bit angles - edited
		if (pitch < 0)
			pitch += 360;
	}
...
cvar_t	sv_aim_h = {"sv_aim_h", "1", true}; // mankrip - horizontal autoaim
#else
cvar_t	sv_aim_h = {"sv_aim_h", "0.98", true}; // mankrip - horizontal autoaim
#endif
cvar_t	sv_aim = {"sv_aim", "0.93", true}; // mankrip - saving aim value
void PF_aim (void)
{
...
// try all possible entities
	...
	// mankrip - horizontal autoaim - begin
	if (sv_aim_h.value < 1)
		bestdist = sv_aim_h.value;
	else
	// mankrip - horizontal autoaim - end
...
extern int current_protocol; // mankrip - QC Alpha Scale Glow
void PF_makestatic (void)
{
	...
	// mankrip - QC Alpha Scale Glow - begin
	...
	// mankrip - QC Alpha Scale Glow - end
...
// mankrip - begin
			...
			if (Sys_FileOpenRead (va("/ram/%s", p), &h) == -1)
#endif
// mankrip - end
...
// mankrip - begin
#ifdef _arch_dreamcast
			if ((fsize = Sys_FileOpenRead (va("/ram/%s", p), &h)) == -1)
#endif
// mankrip - end
...
// mankrip - begin
#ifdef _arch_dreamcast
				h = Sys_FileOpenWrite(va("/ram/%s", p));
#else
// mankrip - end
...
/* // 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes - removed - start
...
*/ // 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes - removed - end
