...
// TODO: could cache lazily, stored in the entity
	...
	if (currententity->scale1 != currententity->scale2 && r_interpolation.value)
	{
		...
		else if (((float)cl.time - currententity->scale_start_time) >= 0.1f) // a interpolação acabou
		{
			blend = currententity->scale2;
			currententity->scale_start_time = 0;
			currententity->scale1 = currententity->scale2; // garantir que os 2 tão do mesmo tamanho
		}
		else
			blend = currententity->scale1 +
				(
