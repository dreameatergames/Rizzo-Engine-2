...
void CL_ParseTEnt (void)
{
	...
	case TE_EXPLOSION:			// rocket explosion
		...
		if ( (int)r_light_fx.value & 16) // mankrip
		{ // mankrip
			dl = CL_AllocDlight (0);
...
entity_t *CL_NewTempEntity (void)
{
	...
	ent->colormap = colorshadingmap_beam;//vid.colormap; // mankrip - edited
	return ent;
}
...
// update lightning
	...
	// calculate pitch and yaw
		...
		{
			yaw = /*(int)*/ (atan2(dist[1], dist[0]) * 180 / M_PI); // mankrip - edited
			if (yaw < 0)
				yaw += 360;

			forward = sqrt (dist[0]*dist[0] + dist[1]*dist[1]);
			pitch = /*(int)*/ (atan2(dist[2], forward) * 180 / M_PI); // mankrip - edited
			if (pitch < 0)
				pitch += 360;
		}
	...
	// add new entities for the lightning
		...
		//	ent->effects |= EF_FULLBRIGHT;
			...
			for (j=0 ; j<3 ; j++) // GoldQuake fix - edited
				org[j] += dist[j]*30; // GoldQuake fix - edited
			d -= 30;
		}
	}
